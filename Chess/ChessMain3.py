...
def main(player):
    ...
    loadimages()  # Runs only once before the loop to avoid unnecessary loading and slowdowns
    sqSelected = () # Stores the user's last click
    playerClicks = [] # Stores the user's clicks
    running = True
    gameover = False
    playerone = False # True if a human plays
    ...
    while running:   # Mouse input is handled inside the while loop
        ...
        for e in p.event.get():
            if e.type == p.QUIT:
                running = False
            elif e.type == p.MOUSEBUTTONDOWN:  # Mouse move starts here
                if not gameover and humanturn:
                    location = p.mouse.get_pos()  # Horizontal or vertical position
                    col = location[0] // SQ_SIZE
                    row = location[1] // SQ_SIZE
                    if sqSelected == (row, col) or col >= 8:  # If the user selects the same square again, reset selections
                        sqSelected = ()
                        playerClicks = []
                    else:
                        sqSelected = (row, col)
                        playerClicks.append(sqSelected)
                    if len(playerClicks) == 2:# If the player has selected a destination (second click)
                        ...
                        for i in range(len(validmoves)):
                            if move == validmoves[i]:
                                ...
                                animate = True
                                sqSelected = ()
                                playerClicks = []  # Reset for the player's next turn
                        if not movemade:
        ...
        drawgamestate(screen, gs, validmoves, sqSelected, movelogfont, player, white_time, black_time)
        if gs.checkmate or gs.stalemate:
            gameover = True
            if gs.stalemate:
                text = 'mate'
            elif gs.whiteToMove:
                text = 'Black_AI wins by Checkmate'
            else:
                text = player + ' wins by Checkmate'
            drawendgametext(screen, text)
        clock.tick(MAX_FPS)
        p.display.flip()

'''
Highlights the selected piece and its possible moves
'''
...
def drawgamestate(screen, gs, validmoves, sqSelected, font, player, white_time, black_time):
    drawboard(screen)
    highlightsquares(screen, gs, validmoves, sqSelected)
    drawpieces(screen, gs.board)
    drawmovelog(screen, gs, font, player)
    AI_text = font.render("Black_AI = ", True, p.Color("white"))
    player_text = font.render(player + " = ", True, p.Color("white"))
    screen.blit(AI_text, (BOARD_WIDTH + MOVE_LOG_PANEL_WIDTH - 100, 25))
    screen.blit(player_text, (BOARD_WIDTH + MOVE_LOG_PANEL_WIDTH - 100, 45))
...
def drawmovelog(screen, gs, font, player):
    ...
    for i in range(0, len(movelog), 2):
        ...
        if i % (movesperrow * 2) == 0:
            player_names = "{} vs Black_AI".format(player)
            ...
            player_time_location = movelogrect.move(253, 5)
            if gs.whiteToMove:
                player_turn = "{}'s Turn".format(player)
            else:
                player_turn = "Black_AI's Turn"
