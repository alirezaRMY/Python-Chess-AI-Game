...
def main(player):
    ...
    running = True
    gameover = False
    playerone = True # True if a human is playing
    playertwo = False # Same as above, but for the black pieces
        ...
        for e in p.event.get():
            if e.type == p.QUIT:
                ...
                if not gameover and humanturn:
                    ...
                    if sqSelected == (row, col) or col >= 8:  # If the user selects the same square again, the variables should be reset
                        sqSelected = ()
                        playerClicks = []
                    else:
                        sqSelected = (row, col)
...
def drawgamestate(screen, gs, validmoves, sqSelected, font, player, white_time, black_time):
    ...
    AI_text = font.render("AI = ", True, p.Color("white"))
    player_text = font.render(player + " = ", True, p.Color("white"))
    screen.blit(AI_text, (BOARD_WIDTH + MOVE_LOG_PANEL_WIDTH - 70, 25))
    screen.blit(player_text, (BOARD_WIDTH + MOVE_LOG_PANEL_WIDTH - 85, 45))
...
def drawmovelog(screen, gs, font, player):
    ...
    for i in range(0, len(movelog), 2):
        ...
        if i % (movesperrow * 2) == 0:
            player_names = "{} vs AI".format(player)
